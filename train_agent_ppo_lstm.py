...
for i_episode in range(0, params['num_episodes']):
    ...
    while not done:
        ...
        action, log_prob, h_out, c_out = player.act(observed_map, robot_pose, h_in, c_in)

        observed_map_prime, robot_pose_prime, reward, done = grid_env.step(action.detach().cpu().numpy()[0][0])

        player.memory.put_data([[observed_map], [robot_pose], [action], [reward], [observed_map_prime],
                                [robot_pose_prime], [log_prob], done, h_in.detach().cpu().numpy()[0],
                                c_in.detach().cpu().numpy()[0], h_out.detach().cpu().numpy()[0],
